...
import numpy as np
from tangoflux.model_pitch_velocity import TangoFlux
...
class TangoFluxInference:
    ...
    def generate(
        ...
        # 修改为
        velocity = max(int(int(velocity/25)-1), 0)
        velocity = min(velocity, 4)  # 使用已经计算的值
        with torch.no_grad():
            latents = self.model.inference_flow(
                ...
                seed=seed if seed is not None else random.randint(1000, 1000000),
            )
