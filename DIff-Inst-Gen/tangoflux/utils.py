...
class Text2AudioDataset(Dataset):
    def __init__(
        self, dataset, prefix, text_column, audio_column, duration, pitch_column, velocity_column, num_examples=-1
    ):
        ...
        self.durations = list(dataset[duration])
        self.indices = list(range(len(self.inputs)))
        self.pitchs = list(dataset[pitch_column])
        self.velocities = list(dataset[velocity_column])
        # velocites contain 5 values, convert 25->0, 50->1, 75->2, 100->3, 127->4
        self.velocities = [int(v/25)-1 for v in self.velocities]


        self.mapper = {}
        for index, audio, duration, text ,pitch, velocity in zip(
            self.indices, self.audios, self.durations, inputs, self.pitchs, self.velocities
        ):
            self.mapper[index] = [audio, text, duration, pitch, velocity]
        ...
        if num_examples != -1:
            self.inputs, self.audios, self.durations = (
                ...
                self.durations[:num_examples],
                self.pitchs[:num_examples],
                self.velocities[:num_examples],
            )
            self.indices = self.indices[:num_examples]
