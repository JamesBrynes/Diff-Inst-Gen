...
class TangoFlux(nn.Module):
    ...
    def __init__(self, config, text_encoder_dir=None, initialize_reference_model=False,):
        ...
        # pitch is int from 0-127
        self.pitch_embedding = nn.Embedding(128, self.in_channels)
        # velocity have 5 values: 25, 50, 75, 100, 127 
        self.velocity_embedding = nn.Embedding(5, self.in_channels)
        # 把512维向量拓展到1024
        self.mulan_embedding = nn.Linear(512, 1024)
        # 可训练参数，用于替代mulan（当mulan为None时使用，让模型只通过文本控制音色）
    ...
    @torch.no_grad()
    def encode_text(self, prompt, num_samples_per_prompt):
        device = self.text_encoder.device
        batch = self.tokenizer(
            prompt,
    ...
    def inference_flow(
        ...
        # 注意：不要用 `seed=random.randint(...)` 作为默认参数，
        # 因为默认参数会在函数定义（import）时求值，从而导致每次调用都用同一个 seed。
        if seed is None:
        ...
        if mulan is not None:
            mulan_hidden_states = self.mulan_embedding(mulan.unsqueeze(1).to(device))
        else:
            print("Using mulan replacement")
            # 使用可训练参数替代mulan，让模型只通过文本控制音色
            mulan_hidden_states = self.mulan_replacement.repeat(bsz, 1).unsqueeze(1).to(device)
        encoder_hidden_states = torch.cat(
            [encoder_hidden_states, duration_hidden_states,mulan_hidden_states], dim=1
        )  ## (bs,seq_len,dim) #[1,4,1024],[1,1,1024] #[1,1,1024]
        # import pdb
        # pdb.set_trace()
        ...
        for i, t in enumerate(timesteps):
            ...
            # 创建latents_input，在classifier_free_guidance时会有两个维度
            latents_input = (
            ...
            # 处理pitch和velocity嵌入
            if classifier_free_guidance:
                # 只在第一个维度应用pitch和velocity嵌入
                # 为第一个维度创建pitch和velocity嵌入
                pitch_tensor = torch.tensor(pitch, device=device).unsqueeze(1)
                velocity_tensor = torch.tensor(velocity, device=device).unsqueeze(1)
                
                pitch_embedding = self.pitch_embedding(pitch_tensor)
                velocity_embedding = self.velocity_embedding(velocity_tensor)
                
                # 扩展到音频序列长度
                pitch_embedding = pitch_embedding.expand(-1, self.audio_seq_len, -1)
                velocity_embedding = velocity_embedding.expand(-1, self.audio_seq_len, -1)
                
                # 为第二个维度创建零嵌入（不应用任何条件信息）
                
                # zero_pitch_embedding = torch.zeros_like(pitch_embedding)
                # zero_velocity_embedding = torch.zeros_like(velocity_embedding)
                zero_pitch_embedding = pitch_embedding
                zero_velocity_embedding = velocity_embedding
                
                # # 合并两个维度的嵌入
                # full_pitch_embedding = torch.cat([pitch_embedding, zero_pitch_embedding])
                # full_velocity_embedding = torch.cat([velocity_embedding, zero_velocity_embedding])
                full_pitch_embedding = torch.cat([zero_pitch_embedding,pitch_embedding])
                full_velocity_embedding = torch.cat([zero_velocity_embedding,velocity_embedding])
            else:
                # 非classifier_free_guidance情况下的处理
                pitch_tensor = torch.tensor(pitch, device=device).unsqueeze(1)
                velocity_tensor = torch.tensor(velocity, device=device).unsqueeze(1)
                
                full_pitch_embedding = self.pitch_embedding(pitch_tensor)
                full_velocity_embedding = self.velocity_embedding(velocity_tensor)
                
                # 扩展到音频序列长度
                full_pitch_embedding = full_pitch_embedding.expand(-1, self.audio_seq_len, -1)
                full_velocity_embedding = full_velocity_embedding.expand(-1, self.audio_seq_len, -1)
            
            # 将嵌入添加到latents_input
            latents_input = latents_input + full_pitch_embedding + full_velocity_embedding
    ...
    # 在forward方法的sft模式下修改pitch和velocity嵌入处理
    def forward(self, latents, prompt, pitch, velocity, mulan, duration=torch.tensor([10]), sft=True):
        ...
        num_samples_per_prompt = 1
        encoder_hidden_states, boolean_encoder_mask = self.encode_text(prompt, num_samples_per_prompt)
        # if random.random() < 0.15:
        #     encoder_hidden_states = torch.zeros_like(encoder_hidden_states)
        #     boolean_encoder_mask = torch.zeros_like(boolean_encoder_mask)
        ...
        # 根据mulan是否为None决定使用mulan嵌入还是可训练参数
        if mulan is not None:
            # 如果有mulan，在训练时随机选择使用mulan或可训练参数（让模型学习不依赖mulan）
            if sft and random.random() < 0.5:
                # 50%概率使用可训练参数，训练模型不依赖mulan也能生成音色
                mulan_hidden_states = self.mulan_replacement.repeat(bsz, 1).unsqueeze(1).to(device)
            else:
                # 使用mulan嵌入
                mulan_hidden_states = self.mulan_embedding(mulan.unsqueeze(1))
        else:
            # 如果没有mulan，使用可训练参数替代，让模型只通过文本控制音色
            mulan_hidden_states = self.mulan_replacement.repeat(bsz, 1).unsqueeze(1).to(device)
        ...
        if sft:
            ...
            # Add pitch and velocity embeddings
            pitch_embeds = self.pitch_embedding(pitch)
            velocity_embeds = self.velocity_embedding(velocity)
            
            # 添加pitch和velocity的无条件训练机制
            if self.uncondition:
                # 为每个样本随机决定是否进行无条件训练
                for i in range(bsz):
                    # 以0.1概率将pitch嵌入设为零
                    if random.random() < 0.1:
                        pitch_embeds[i] = 0
                    # 以0.1概率将velocity嵌入设为零
                    if random.random() < 0.1:
                        velocity_embeds[i] = 0
            
            # expand pitch and velocity embeddings to match audio sequence length [4,1,64]->[4,107,64]
            pitch_embeds = pitch_embeds.expand(-1, audio_seq_length, -1)
            velocity_embeds = velocity_embeds.expand(-1, audio_seq_length, -1)

            # Add pitch and velocity embeddings to noisy model input
            noisy_model_input = noisy_model_input + pitch_embeds + velocity_embeds
