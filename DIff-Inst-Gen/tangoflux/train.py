...
from model_pitch_velocity import TangoFlux
from datasets import load_dataset, Audio
from utils_pitch_velocity import Text2AudioDataset, read_wav_file, pad_wav
...
def main():
    ...
    # Configure DDP to handle unused parameters (needed when mulan_mask is conditionally used)
    ...
    if resume_from_checkpoint and resume_from_checkpoint != "":
        # Allow loading older checkpoints that don't yet contain newly-added parameters
        # (e.g. "mulan_replacement") without failing.
    ...
    for epoch in range(starting_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                optimizer.zero_grad()
                device = model.device
                text, audios, duration, _, pitch, velocity = batch
                ...
                with torch.no_grad():
                    ...
                    for audio_path in audios:
                        ...
                        mulan = torch.load(audio_path.replace('/audio','/mulan').replace('.wav','.pt')).cpu()
                        if (
                            wav.shape[0] == 1
                        ):  ## If this audio is mono, we repeat the channel so it become "fake stereo"
                            wav = wav.repeat(2, 1)
                        audio_list.append(wav)
                        mulan_list.append(mulan)
                    ...
                    mulan_latent = torch.stack(mulan_list, dim=0).to(device)

                loss, _, _, _ = model(audio_latent, text, pitch, velocity, mulan_latent, duration=duration)
            ...
            if isinstance(checkpointing_steps, int):
                if completed_steps % checkpointing_steps == 0:
                    output_dir_2 = f"step_{completed_steps}"
                    if output_dir is not None:
                        output_dir3 = os.path.join(output_dir, output_dir_2)
                    accelerator.save_state(output_dir3)
        ...
        for step, batch in enumerate(eval_dataloader):
            with accelerator.accumulate(model) and torch.no_grad():
                ...
                val_loss, _, _, _ = model(audio_latent, text, pitch, velocity, mulan_latent, duration=duration)
